...
public func setAppLogLevel(_ logLevel: AppLogLevel) {
    Logger.shared.level = logLevel
}

internal final class Logger {
    fileprivate var level: AppLogLevel = .warn

    fileprivate let osLog: OSLog = .init(subsystem: "co.daily.openSesame", category: "main")
    ...
    @inlinable
    internal func log(_ level: AppLogLevel, _ message: @autoclosure () -> String) {
